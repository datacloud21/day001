...
def ping_ip(srclis, dstlis, keylis):
    # if  mutex.acquire(1):
    ...
    try:
        ...
        tr(packmin, 'ping_min', keylis)
        tr(packlo, 'ping_lo', keylis)
        tr(packavg, 'ping_avg', keylis)
        tr(packmax, 'ping_max', keylis)
        tr(packdtr, 'ping_dtr', keylis)
        # print("ping_ip->keylist[i]",keylist[i])
        # mutex.release()
    except RuntimeError:
        print('now error is>>>>>>>>>>>>>>>>>>>>>>>>', srclist[i], dstlist[i], keylist[i])
        os._exit(0)
...
for i in range(len(linelist)):
    ...
    try:
        ...
        t = threading.Thread(target=ping_ip, args=(srclist[i], dstlist[i], keylist[i]))
        t.setDaemon(True)
        threads.append(t)
    except Exception:
        print('error')
